...
function TRegExpr.TableInvertCaseFunction(const Ch : REChar) : REChar;
begin
...
class function TRegExpr.InvertCaseFunction (const Ch : REChar) : REChar;
 ...
 {$IFDEF UniCode}
   ...
   if Result = Ch then Result := UnicodeLowerCase(Ch)[1];
 {$ELSE}
        ...
        case Mode of
          smodeOneLower, smodeAllLower:
            begin
              Ch := p0^;
              {$IFDEF UNICODE}
              Ch := UnicodeLowerCase(Ch)[1];
              {$ELSE}
              Ch := Chr(FLowerCase[Ord(Ch)]);
              {$ENDIF}
              ResultPtr^ := Ch;
          ...
          smodeOneUpper, smodeAllUpper:
            begin
              Ch := p0^;
              {$IFDEF UNICODE}
              Ch := UnicodeUpperCase(Ch)[1];
              {$ELSE}
              Ch := Chr(FUpperCase[Ord(Ch)]);
              {$ENDIF}
              ResultPtr^ := Ch;
...
function TRegExpr.ExecRegExpr(const ARegExpr, AInputStr: RegExprString): Boolean;
begin
  Self.Expression:= ARegExpr;
  Result:= Self.Exec(AInputStr);
end;

function TRegExpr.ReplaceRegExpr(const ARegExpr, AInputStr,
